...
class Database
{
	 ...
	 * @param Error $error Injected Error instance.
	 */
	public function __construct(Logger $logger, Error $error)
	{
		$this->logger = $logger;
		$this->error = $error;
		...
		if (empty($config['DB_USERNAME'])) {
			$this->error->terminateWithError('DB_USERNAME_NOT_PROVIDED');
		}
		
		if (empty($config['DB_PASSWORD'])) {
			$this->error->terminateWithError('DB_PASSWORD_NOT_PROVIDED');
		}
		...
		try {
			...
			$this->error->terminateWithError('DATABASE_CONNECTION_FAILED', $e->getMessage());
		}
	}
	...
	private function getDsn(array $config): string
	{
		if (empty($config['DB_TYPE'])) {
			$this->error->terminateWithError('DB_TYPE_NOT_PROVIDED');
		}
		
		if (empty($config['DB_HOST'])) {
			$this->error->terminateWithError('DB_HOST_NOT_PROVIDED');
		}
		
		if (empty($config['DB_NAME'])) {
			$this->error->terminateWithError('DB_NAME_NOT_PROVIDED');
		}
		
		if (empty($config['DB_PORT'])) {
			$this->error->terminateWithError('DB_PORT_NOT_PROVIDED');
		}
		...
		switch ($dbType) {
			...
			case 'sqlite':
				...
				break;
			default:
				$this->error->terminateWithError('UNSUPPORTED_DB_TYPE', $dbType);
		}
	...
	public function sanitizeInput($input, string $type = 'string')
	{
		switch ($type) {
			case 'string':
				// FILTER_SANITIZE_STRING is deprecated in PHP 8.1, using SPECIAL_CHARS instead
	...
	public function handleConnectionTimeout(): void
	{
		...
		try {
			...
			$this->error->terminateWithError('DATABASE_CONNECTION_TIMEOUT', $e->getMessage());
		}
	}
	...
	public function connectWithFailover(): void
	{
		...
		try {
			...
			try {
				...
				$this->error->terminateWithError('DATABASE_CONNECTION_FAILOVER_FAILED', $e->getMessage());
			}
		}
	}
	...
	public function queryBuilder(): QueryBuilder
	{
		...
		if (empty($config['QUERY_BUILDER_ENABLED']) || $config['QUERY_BUILDER_ENABLED'] !== 'true') {
			$this->error->terminateWithError('QUERY_BUILDER_NOT_ENABLED');
		}
	...
	public function connectToShard(string $shardKey): void
	{
		...
		if (!$shardingConfigPath || !file_exists($shardingConfigPath)) {
			$this->error->terminateWithError('SHARDING_CONFIG_NOT_PROVIDED');
		}
		...
		try {
			...
			$this->error->terminateWithError('SHARDING_CONNECTION_FAILED', $e->getMessage());
		}
	}
	...
	public function validateSchema(array $schema): bool
	{
		...
		try {
			...
			foreach ($schema as $table => $columns) {
				...
				foreach ($columns as $column) {
					if (!in_array($column, $existingColumns)) {
						...
						$this->error->terminateWithError('SCHEMA_VALIDATION_FAILED', "Column $column not found in table $table.");
					}
				}
			}
			...
			$this->error->terminateWithError('SCHEMA_VALIDATION_FAILED', $e->getMessage());
		}
	...
	public function cacheTableMetadata(string $table, CacheInterface $cache): array
	{
		...
		try {
			...
			$this->error->terminateWithError('METADATA_CACHE_FAILED', $e->getMessage());
		}
		...
		while ($attempts < $maxAttempts) {
			try {
				...
				if ($attempts >= $maxAttempts) {
					...
					$this->error->terminateWithError('QUERY_RETRY_FAILED', $e->getMessage());
				}
	...
	public function monitorServerStatus(): bool
	{
		...
		try {
			...
			if ($status && isset($status['Value']) && $status['Value'] > 0) {
				...
				$this->error->terminateWithError('SERVER_STATUS_CHECK_FAILED', 'Server is not running.');
			}
		} catch (PDOException $e) {
			$this->logger->logError("Server status check failed: " . $e->getMessage());
			$this->error->terminateWithError('SERVER_STATUS_CHECK_FAILED', $e->getMessage());
		}
		...
		try {
			...
			if ($logOutput && isset($logOutput['Value']) && $logOutput['Value'] === 'FILE') {
				...
				if ($logFile && isset($logFile['Value'])) {
					...
					$this->error->terminateWithError('SERVER_LOG_MANAGEMENT_FAILED', 'Unable to retrieve current log file path.');
				}
			} else {
				$this->logger->logError("Server log management failed: Log output is not set to FILE.");
				$this->error->terminateWithError('SERVER_LOG_MANAGEMENT_FAILED', 'Log output is not set to FILE.');
			}
		} catch (PDOException $e) {
			$this->logger->logError("Server log management failed: " . $e->getMessage());
			$this->error->terminateWithError('SERVER_LOG_MANAGEMENT_FAILED', $e->getMessage());
		}
	...
	public function manageServerBackups(string $backupDirectory): bool
	{
		...
		try {
			...
			if ($returnVar !== 0) {
				...
				$this->error->terminateWithError('SERVER_BACKUP_FAILED', implode("\n", $output));
			}
			...
			$this->error->terminateWithError('SERVER_BACKUP_FAILED', $e->getMessage());
		}
	...
	public function manageServerReplication(string $replicationConfigFile): bool
	{
		...
		try {
			...
			if (!file_exists($replicationConfigFile)) {
				...
				$this->error->terminateWithError('SERVER_REPLICATION_FAILED', 'Replication configuration file not found.');
			}
			...
			if (json_last_error() !== JSON_ERROR_NONE) {
				...
				$this->error->terminateWithError('SERVER_REPLICATION_FAILED', 'Invalid JSON in replication configuration file.');
			}
			...
			$this->error->terminateWithError('SERVER_REPLICATION_FAILED', $e->getMessage());
		}
